...
COLORBAR_ON=true;
FE_BOUNDARY=false;
SHOW_REC_POSITIONS=false;

SAVE_SEISMOGRAMS=false;
% seis_tag='mz2';
% seis_tag='pconvsin1000';
seis_tag=['pflattriso2D' num2str(NX)];
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
eps=0.00000000001d0;
...
memory_dvx_dx = zeros(size(sigmaxx));
memory_dvy_dy_2 = zeros(size(sigmaxx));
memory_dvx_dx_2 = zeros(size(sigmaxx));
memory_dvy_dy = zeros(size(sigmaxx));
memory_dvy_dx = zeros(size(sigmaxx));
memory_dvx_dy_2 = zeros(size(sigmaxx));
memory_dvy_dx_2 = zeros(size(sigmaxx));
memory_dvx_dy = zeros(size(sigmaxx));

memory_dsigmaxx_dx = zeros(size(sigmaxx));
memory_dsigmaxy_dy = zeros(size(sigmaxx));
memory_dsigmayx_dx = zeros(size(sigmaxx));
memory_dsigmayy_dy = zeros(size(sigmaxx));
memory_dsigmaxy_dx = zeros(size(sigmaxx));

ax = zeros(1,NX+1,NY+1);
ay = zeros(1,NX+1,NY+1);
vx = zeros(1,NX+1,NY+1);
vy = zeros(1,NX+1,NY+1);
...
%             memory_dux_dxy(i,j) = b_x(j) * memory_dux_dxy(i,j) + a_x(j) * value_dux_dxy;
%             memory_duy_dxy(i,j) = b_y(j) * memory_duy_dxy(i,j) + a_y(j) * value_duy_dxy;
%             memory_duy_dxy(i,j) = b_x(j) * memory_duy_dxy(i,j) + a_x(j) * value_duy_dxy;
%             
...
%                 if ((i==ISOURCE && j==JSOURCE) || (i==ISOURCE && j==JSOURCE+1)) && it>=6
%                     fprintf('ux: S: %e\n',ux(3,i,j));
%                     if (i==ISOURCE && j==JSOURCE+2)
%                         fprintf('\n');
%                     end
%                 end
...
%             c44v=C(i,j,4);
%             
%             value_dux_dx = (ux(2,i+1,j) - ux(2,i-1,j)) / ddx;
%             value_duy_dx = (uy(2,i+1,j) - uy(2,i-1,j)) / ddx;
%             
%             value_dux_dy = (ux(2,i,j+1) - ux(2,i,j-1)) / ddy;
%             value_duy_dy = (uy(2,i,j+1) - uy(2,i,j-1)) / ddy;
%             
%             sigmaxx(i,j) = c11v*value_dux_dx + c13v*value_duy_dy;
%             sigmaxy(i,j) = c44v*value_dux_dy + c44v*value_duy_dx;
%             sigmayx(i,j) = c44v*value_duy_dx + c44v*value_dux_dy;
%             sigmaxx(i,j) = c13v*value_dux_dx + c33v*value_duy_dy;
%         end
%     end
%     
%     for i = 2:NX
...
%             dt2rho=(DELTAT^2.d0)/rhov;
%             
%             dsigmaxx_dx = (sigmaxx(i+1,j) - sigmaxx(i-1,j)) / ddx;
%             dsigmaxy_dy = (sigmaxy(i,j+1) - sigmaxy(i,j-1)) / ddy;
%             dsigmayx_dx = (sigmayx(i+1,j) - sigmayx(i-1,j)) / ddx;
%             dsigmayy_dy = (sigmayy(i,j+1) - sigmayy(i,j-1)) / ddy;
%             
%             ux(3,i,j) = 2.d0 * ux(2,i,j) - ux(1,i,j) + (dsigmaxx_dx + dsigmaxy_dy) * dt2rho;
%             uy(3,i,j) = 2.d0 * uy(2,i,j) - uy(1,i,j) + (dsigmayx_dx + dsigmayy_dy) * dt2rho;
%         end
%     end
...
for j = 2:NY
    for i = 2:NX
      ...
      value_dvx_dx = (ux(2,i+1,j) - ux(2,i,j)) / DELTAX;
      value_dvy_dy = (uy(2,i,j+1) - uy(2,i,j)) / DELTAY;
      value_dvx_dx_2 = (ux(2,i+1,j+1) - ux(2,i-1,j+1)+ux(2,i+1,j)-ux(2,i-1,j)) / DELTAX*0.25d0;
      value_dvy_dy_2 = (uy(2,i+1,j+1) - uy(2,i+1,j-1)+uy(2,i,j+1)-uy(2,i,j-1)) / DELTAY*0.25d0;

      memory_dvx_dx(i,j) = b_x(i) * memory_dvx_dx(i,j) + a_x(i) * value_dvx_dx;
      memory_dvy_dy_2(i,j) = b_y(j) * memory_dvy_dy_2(i,j) + a_y(j) * value_dvy_dy_2;
      memory_dvx_dx_2(i,j) = b_x(i) * memory_dvx_dx_2(i,j) + a_x(i) * value_dvx_dx_2;
      ...
      value_dvx_dx_2 = value_dvx_dx_2 / K_x(i) + memory_dvx_dx_2(i,j);
      value_dvy_dy_2 = value_dvy_dy_2 / K_y(j) + memory_dvy_dy_2(i,j);

      sigmaxx(i,j) = c11v * value_dvx_dx + c13v * value_dvy_dy_2;
      sigmayy(i,j) = c13v * value_dvx_dx_2 + c33v * value_dvy_dy;
    end
end
  ...
  for j = 2:NY
    for i = 2:NX
      ...
      value_dvy_dx = (uy(2,i+1,j) - uy(2,i,j)) / DELTAX;
      value_dvx_dy = (ux(2,i,j+1) - ux(2,i,j)) / DELTAY;
      value_dvy_dx_2 = (uy(2,i+1,j+1) - uy(2,i-1,j+1)+uy(2,i+1,j)-uy(2,i-1,j)) / DELTAX*0.25d0;
      value_dvx_dy_2 = (ux(2,i+1,j+1) - ux(2,i+1,j-1)+ux(2,i,j+1)-ux(2,i,j-1)) / DELTAY*0.25d0;

      memory_dvy_dx(i,j) = b_x(i) * memory_dvy_dx(i,j) + a_x(i) * value_dvy_dx;
      memory_dvx_dy_2(i,j) = b_y(j) * memory_dvx_dy_2(i,j) + a_y(j) * value_dvx_dy_2;
      memory_dvy_dx_2(i,j) = b_x(i) * memory_dvy_dx_2(i,j) + a_x(i) * value_dvy_dx_2;
      memory_dvx_dy(i,j) = b_y(j) * memory_dvx_dy(i,j) + a_y(j) * value_dvx_dy;

      value_dvy_dx = value_dvy_dx / K_x(i) + memory_dvy_dx(i,j);
      value_dvx_dy = value_dvx_dy / K_y(j) + memory_dvx_dy(i,j);

      value_dvy_dx_2 = value_dvy_dx_2 / K_x(i) + memory_dvy_dx_2(i,j);
      value_dvx_dy_2 = value_dvx_dy_2 / K_y(j) + memory_dvx_dy_2(i,j);

      sigmaxy(i,j) =  c44v * (value_dvx_dy + value_dvy_dx_2);
      sigmayx(i,j) =  c44v * (value_dvy_dx + value_dvx_dy_2);
    end
  end
...
%--------------------------------------------------------
  for j = 2:NY
    for i = 2:NX
      ...
      ax(1,i,j) = (value_dsigmaxx_dx + value_dsigmaxy_dy)/ rho(i,j);
    end
  end

  for j = 2:NY
    for i = 2:NX
      value_dsigmaxy_dx = (sigmayx(i,j) - sigmayx(i-1,j)) / DELTAX;
      value_dsigmayy_dy = (sigmayy(i,j) - sigmayy(i,j-1)) / DELTAY;
      ...
      ay(1,i,j)=(value_dsigmaxy_dx + value_dsigmayy_dy)/rho(i,j);
    end
  end
...
%     ux(3,i,j) = ux(3,i,j) + force_x * DELTAT / rho(i,j);
%     uy(3,i,j) = uy(3,i,j) + force_y * DELTAT / rho(i,j);

%       ax(1,i,j) = ax(1,i,j) + force_x / rho(i,j);
%       ay(1,i,j) = ay(1,i,j) + force_y / rho(i,j);
%       
      vx = vx + ax * DELTAT;
      vy = vy + ay * DELTAT;
      
      ux(3,:,:) = ux(3,:,:) + DELTAT * vx;% +  force_x * DELTAT / rho(i,j);
      uy(3,:,:) = uy(3,:,:) + DELTAT * vy;% +  force_x * DELTAT / rho(i,j);
