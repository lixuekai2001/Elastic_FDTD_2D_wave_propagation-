...
%  NSTEP = 1500;
 ...
 model = load('marmhard.dat');
 model = reshape(model,122,384);
 model = model';
...
maxcp =max(max(model));
Courant_number = maxcp * DELTAT * sqrt(1.d0/DELTAX^2.d0 + 1.d0/DELTAY^2.d0);
...
SHOW_UX =false; %doesn't work, because I didn't pay attention to it yet
SHOW_UY =true;
DISP_NORM=false;
...
%                 end
    ...
    if(mod(it,IT_DISPLAY) == 0 || it == 5)
        ...
        if(SHOW_UX || SHOW_UY)
            ...
            u_plot = squeeze(u(1,:,:))';
            scaling = 3* maxcp/max(max(squeeze(u(1,:,:))));
%             colormap jet;
%             fig_model = imagesc(nx_vec,ny_vec,flipud(model')); hold on; freezeColors;
%             colormap(CMAP);
%             fig_wf = imagesc(nx_vec,ny_vec,scaling*u_plot);
              fig_wf = imagesc(nx_vec,ny_vec, u_plot);
%             alpha(fig_wf,.5); freezeColors;
%             alpha_mask = u_plot < 0.001*u_plot;
%             set(fig_wf, 'AlphaData', alpha_mask); hold on;
