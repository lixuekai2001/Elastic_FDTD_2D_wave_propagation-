...
%
% Conventional FD star-stencils deliver accuracy O(2,4)
...
ux = zeros(3,nx+4,nz+4);            % Wavefields for time steps
uz = zeros(3,nx+4,nz+4);            % t-2 (1), t-1 (2) and t (3)
...
for it = 1:nt                       % Loop over TIME
    ...
    uz(3,:,:)=0.0;
    for i = 4:nx                    % Loop over OX
        for j = 4:nz                % Loop over OZ
            lam = lambda(i,j);
            mu = muu(i,j);
            lam_2mu = lam + 2 * mu;
