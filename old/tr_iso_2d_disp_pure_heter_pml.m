...
SNAPSHOT=false;
snapshot_time=700:100:4000; %on what steps
...
% seis_tag='mz2';
% seis_tag='ex0';
seis_tag=['hcurvetriso' num2str(NX)];
...
memory_dux_dyx=zeros(NX+1,NY+1);
memory_duy_dyx=zeros(NX+1,NY+1);
...
%             
%             memory_dux_dyx(i,j) = b_y(j) * memory_dux_dyx(i,j) + a_y(j) * value_dux_dyx;
%             memory_dux_dyx(i,j) = b_x(j) * memory_dux_dyx(i,j) + a_x(j) * value_dux_dyx;
%             memory_duy_dyx(i,j) = b_y(j) * memory_duy_dyx(i,j) + a_y(j) * value_duy_dyx;
%             memory_duy_dyx(i,j) = b_x(j) * memory_duy_dyx(i,j) + a_x(j) * value_duy_dyx;
%             
%             value_dux_dyx = value_dux_dyx / K_x(i) + memory_dux_dyx(i,j);
%             value_dux_dyx = value_dux_dyx / K_y(j) + memory_dux_dyx(i,j);
%             value_duy_dyx = value_duy_dyx / K_x(i) + memory_duy_dyx(i,j);
%             value_duy_dyx = value_duy_dyx / K_y(j) + memory_duy_dyx(i,j);

            %--------------------------------------------------------------------------------------------------------------------
            
                dt2rho=(DELTAT^2.d0)/rhov;
                
                sigmaxx_dx = value_dux_dxx + value_duy_dyx;
                sigmaxy_dy = value_dux_dyy + value_duy_dxy;
                sigmayx_dx = value_dux_dyx + value_duy_dxx;
                sigmayy_dy = value_dux_dxy + value_duy_dyy;
 
                ux(3,i,j) = 2.d0 * ux(2,i,j) - ux(1,i,j) + (sigmaxx_dx + sigmaxy_dy) * dt2rho;
                uy(3,i,j) = 2.d0 * uy(2,i,j) - uy(1,i,j) + (sigmayx_dx + sigmayy_dy) * dt2rho;
